...
typeBoilerplate manifest (tname, TypeOpaque c_type_name ops extra_ops) =
  let type_name = typeStructName tname
      aux_name = type_name <> "_aux"
      (record_edecls, record_init, kind) = recordDefs type_name extra_ops
   in ( [C.cedecl|const struct type $id:type_name;|],
        [C.cinit|&$id:type_name|],
        record_edecls
          ++ [C.cunit|
  ...
  where
    ...
    recordDefs type_name (Just (OpaqueSum (SumOps variants variant))) =
      let variant_wrap = variant <> "_wrap"
          sum_name = type_name <> "_sum"
          variants_name = type_name <> "_variants"
          test n = [C.cinit|&$id:(typeStructName n)|]
          onVariant (SumVariant name variant_tnames new _) =
            [C.cinit|{.name = $string:(T.unpack name),
                      .num_types = $int:(length variant_tnames),
                      .types = (const struct type*) { $inits:(map test variant_tnames) },
                      .new = (typename new_fn)$id:new
                    }|]
          variant_inits = map onVariant variants
       in ( [C.cunit|
             const struct variant $id:variants_name[] = {
               $inits:variant_inits
             };
             int $id:variant_wrap(struct futhark_context* ctx, const void* vp) {
               const typename $id:c_type_name v = (const typename $id:c_type_name) vp;
               return $id:variant(ctx, v);
             }
             const struct sum $id:sum_name = {
               .num_variants = $int:(length variants),
               .variants = $id:variants_name,
               .variant = $id:variant_wrap
             };|],
            [C.cinit|&$id:sum_name|],
            "SUM"
          )
    recordDefs _ _ = ([], [C.cinit|NULL|], "SCALAR")
